...
class Application : GameWindowBase
{
    ...
    private CameraMotionBlur? CameraMotionBlurPass;
    private bool IsCameraMotionBlur = true; // 기본 켜두기/끄려면 false
    ...
    protected override void OnRender(float dT)
    {
        ...
        if (RequestPresentationResolution.HasValue || RequestRenderResolutionScale.HasValue)
        {
            ...
            CameraMotionBlurPass?.SetSize(newPresenRes);

            AfterImagePass?.SetSize(newPresenRes);
            if (AfterImagePass != null) AfterImagePass.Settings.ResetHistory = true;
        }
        ...
        BBG.Texture final = TonemapAndGamma.Result;

        // 카메라 기반 모션블러 (camera-only 느낌)
        if (IsCameraMotionBlur && CameraMotionBlurPass != null)
        {
            BBG.Texture? depth = null;

            if (RenderMode_ == RenderMode.Rasterizer)
            {
                depth = RasterizerPipeline?.DepthTexture;

                if (depth == null)
                {
                    Logger.Log(Logger.LogLevel.Warn, "MotionBlur: Rasterizer DepthTexture is NULL");
                }
            }

            {
                // 세팅값
                CameraMotionBlurPass.Settings.Strength = 2.0f; // default = 1.0f
                CameraMotionBlurPass.Settings.MaxBlurPixels = 32.0f; // default = 16.0f
                CameraMotionBlurPass.Settings.Samples = 16; // default = 12

                CameraMotionBlurPass.Compute(final, depth, gpuPerFrameData.PrevProjView, gpuPerFrameData.InvProjView);
                final = CameraMotionBlurPass.Result;
            }
        }


        // 잔상(가짜 모션블러) 적용
        if (IsAfterImage && AfterImagePass != null)
        {
            AfterImagePass.Compute(final);
            final = AfterImagePass.Result;
        }
        ...
        {
            BBG.Rendering.CopyTextureToSwapchain(final);
        }
    ...
    private void SetResolutions(Vector2i renderRes, Vector2i presentRes)
    {
        ...
        if (RenderMode_ == RenderMode.Rasterizer || RenderMode_ == RenderMode.PathTracer)
        {
            ...
            if (AfterImagePass != null)
            {
                AfterImagePass.SetSize(presentRes);
                AfterImagePass.Settings.ResetHistory = true;
            }
    ...
    private void SetRenderMode(RenderMode renderMode, Vector2i renderRes, Vector2i presentRes)
    {
        ...
        if (renderMode == RenderMode.Rasterizer || renderMode == RenderMode.PathTracer)
        {
            ...
            var tonemapSettings = TonemapAndGamma != null
                ? TonemapAndGamma.Settings
                : new TonemapAndGammaCorrect.GpuSettings();

            TonemapAndGamma?.Dispose();
            TonemapAndGamma = new TonemapAndGammaCorrect(presentRes, tonemapSettings);

            CameraMotionBlurPass ??= new CameraMotionBlur(presentRes);
            CameraMotionBlurPass.SetSize(presentRes);

            if (AfterImagePass == null)
            {
                AfterImagePass = new AfterImage(
                    new Vector2i(TonemapAndGamma.Result.Width, TonemapAndGamma.Result.Height),
                    new AfterImage.GpuSettings { Alpha = 0.5f, ResetHistory = true } // Alpha 값 조정으로 잔상 강도 조절
                );
            }
            else
            {
                AfterImagePass.SetSize(new Vector2i(TonemapAndGamma.Result.Width, TonemapAndGamma.Result.Height));
                AfterImagePass.Settings.ResetHistory = true;
            }
