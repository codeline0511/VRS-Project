...
class RasterPipeline : IDisposable
{
    ...
    public RasterPipeline(Vector2i renderSize, Vector2i presentationSize)
    {
        ...
        IsSSR = false;
        IsMotionBlur = true;
        IsVariableRateShading = false; // GTX 1050에서는 실행 불가
        IsMotionAwareVRS = false;      // GTX 1050에서는 실행 불가
        IsVXGI = false;
    ...
    public void Render(ModelManager modelManager, LightManager lightManager, Camera camera, float dT, Vector2 mousePos, Vector2 windowSize)
    {
        ...
        BBG.Rendering.VariableRateShadingNV selectedVRSData = IsMotionAwareVRS ? MotionAwareVRS.GetRenderData() : LightingVRS.GetRenderData();
        ...
        }, new BBG.Rendering.GraphicsPipelineState()
        {
            ...
            VariableRateShading = selectedVRSData,
        }, () =>
        {
        ...
        if (IsVariableRateShading || 
            LightingVRS.Settings.DebugValue != LightingShadingRateClassifier.DebugMode.None || 
            MotionAwareVRS.Settings.DebugValue != MotionAwareVRSClassifier.DebugMode.None)
        {
            if (IsMotionAwareVRS)
            {
                var settings = MotionAwareVRS.Settings;

                if (mousePos.X >= 0)
                {
                    Vector2 normalizedMouse = mousePos / windowSize;
                    normalizedMouse.Y = 1.0f - normalizedMouse.Y;
                    settings.MousePos = normalizedMouse;
                }

                MotionAwareVRS.Settings = settings;
                MotionAwareVRS.Compute(beforeTAATexture);
            }
            else
            {
                var settings = LightingVRS.Settings;
                ...
                settings.IsFoveated = 1;
                LightingVRS.Settings = settings;
                LightingVRS.Compute(beforeTAATexture);
            }
        }
        ...
        if (IsMotionBlur)
        {
            var motionBlurSettings = MotionBlur.Settings;

            // 실제 프레임 시간(초)
            motionBlurSettings.DeltaTime = MathF.Max(dT, 1.0f / 1000.0f);

            // 기준 프레임 시간(초): 60 FPS 기준
            motionBlurSettings.ReferenceDeltaTime = 1.0f / 60.0f;

            MotionBlur.Settings = motionBlurSettings;

            // 입력과 출력은 분리
            MotionBlur.Compute(beforeTAATexture, MotionBlur.Result);
        }

        BBG.Texture postMotionBlurTexture = IsMotionBlur ? MotionBlur.Result : beforeTAATexture;

        if (IsSSR)
        {
            SSR.Compute(postMotionBlurTexture);
        }

        BBG.Texture mergeInputTexture = IsMotionBlur ? MotionBlur.Result : beforeTAATexture;

        BBG.Computing.Compute("Merge Textures", () =>
        {
            // 최종 결과는 여전히 beforeTAATexture에 써 넣음
            BBG.Cmd.BindImageUnit(beforeTAATexture, 0);

            // 입력은 모션블러 결과 또는 원본
            BBG.Cmd.BindTextureUnit(mergeInputTexture, 0, mergeInputTexture != null);
