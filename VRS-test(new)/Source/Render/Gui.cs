        ...
        if (ImGui.Begin("Renderer"))
        {
            ...
            if (app.RenderMode_ == Application.RenderMode.Rasterizer)
            {
                ...
                if (ImGui.CollapsingHeader("Variable Rate Shading"))
                {
                    ...
                    CheckBoxEnabled(
                        "IsVariableRateShading",
                        ref app.RasterizerPipeline.IsVariableRateShading,
                        LightingShadingRateClassifier.IS_SUPPORTED
                    );

                    // MotionAwareVRS 사용 여부
                    bool isMotionAwareVRS = app.RasterizerPipeline.IsMotionAwareVRS;
                    CheckBoxEnabled(
                        "Use MotionAwareVRS",
                        ref isMotionAwareVRS,
                        LightingShadingRateClassifier.IS_SUPPORTED
                    );
                    app.RasterizerPipeline.IsMotionAwareVRS = isMotionAwareVRS;

                    ImGui.Separator();

                    if (app.RasterizerPipeline.IsMotionAwareVRS)
                    {
                        var settings = app.RasterizerPipeline.MotionAwareVRS.Settings;

                        // DebugMode Combo
                        if (ImGui.BeginCombo("DebugMode", settings.DebugValue.ToString()))
                        {
                            MotionAwareVRSClassifier.DebugMode[] debugModes = Enum.GetValues<MotionAwareVRSClassifier.DebugMode>();
                            for (int i = 0; i < debugModes.Length; i++)
                            {
                                MotionAwareVRSClassifier.DebugMode mode = debugModes[i];

                                bool isSelected = mode == settings.DebugValue;
                                if (ImGui.Selectable(mode.ToString(), isSelected))
                                {
                                    settings.DebugValue = mode;
                                }
                        ...
                        ImGui.SliderFloat("MotionLow", ref settings.MotionLow, 0.0f, 8.0f);
                        ImGui.SliderFloat("MotionHigh", ref settings.MotionHigh, 0.1f, 16.0f);

                        ImGui.SliderFloat("LumaVarianceLow", ref settings.LumaVarianceLow, 0.00001f, 0.01f);
                        ImGui.SliderFloat("LumaVarianceHigh", ref settings.LumaVarianceHigh, 0.0001f, 0.05f);

                        ImGui.SliderFloat("DepthRangeThreshold", ref settings.DepthRangeThreshold, 0.0001f, 0.1f);

                        bool isFoveated = settings.IsFoveated != 0;
                        ImGui.Checkbox("IsFoveated##MotionAwareVRS", ref isFoveated);
                        settings.IsFoveated = isFoveated ? 1 : 0;

                        ImGui.SliderFloat("InnerRadius", ref settings.InnerRadius, 0.05f, 0.5f);
                        ImGui.SliderFloat("MiddleRadius", ref settings.MiddleRadius, 0.1f, 0.8f);

                        bool allow2x2 = settings.Allow2x2 != 0;
                        ImGui.Checkbox("Allow2x2", ref allow2x2);
                        settings.Allow2x2 = allow2x2 ? 1 : 0;

                        bool allow4x2 = settings.Allow4x2 != 0;
                        ImGui.Checkbox("Allow4x2", ref allow4x2);
                        settings.Allow4x2 = allow4x2 ? 1 : 0;

                        bool allow4x4 = settings.Allow4x4 != 0;
                        ImGui.Checkbox("Allow4x4", ref allow4x4);
                        settings.Allow4x4 = allow4x4 ? 1 : 0;

                        ImGui.SliderFloat("MotionBias", ref settings.MotionBias, 0.1f, 4.0f);
                        ImGui.SliderFloat("VarianceBias", ref settings.VarianceBias, 0.1f, 4.0f);

                        app.RasterizerPipeline.MotionAwareVRS.Settings = settings;
                    }
                    else
                    {
                        var settings = app.RasterizerPipeline.LightingVRS.Settings;
                        ...
                        ImGui.SliderFloat("SpeedFactor", ref settings.SpeedFactor, 0.0f, 1.0f);
                        ImGui.SliderFloat("LumVarianceFactor", ref settings.LumVarianceFactor, 0.0f, 0.3f);

                        bool isFoveated = settings.IsFoveated != 0;
                        ImGui.Checkbox("IsFoveated##LightingVRS", ref isFoveated);
                        settings.IsFoveated = isFoveated ? 1 : 0;

                        app.RasterizerPipeline.LightingVRS.Settings = settings;
                    }
                }
                ...
                if (ImGui.CollapsingHeader("MotionBlur"))
                {
                    ...
                    {
                        var settings = app.RasterizerPipeline.MotionBlur.Settings;

                        ImGui.SliderInt("Samples##SamplesMotionBlur", ref settings.SampleCount, 1, 32);
                        ImGui.SliderFloat("Intensity", ref settings.Intensity, 0.0f, 25.0f);
                        ImGui.SliderFloat("MaxBlurPixels", ref settings.MaxBlurPixels, 1.0f, 128.0f);

                        // 추가된 항목도 GUI에서 조절 가능하게 하려면
                        ImGui.SliderFloat("ShutterScale", ref settings.ShutterScale, 0.0f, 2.0f);
                        ImGui.SliderFloat("MinDeltaScale", ref settings.MinDeltaScale, 0.1f, 2.0f);
                        ImGui.SliderFloat("MaxDeltaScale", ref settings.MaxDeltaScale, 0.5f, 4.0f);

                        app.RasterizerPipeline.MotionBlur.Settings = settings;
                    }
                }
            }
